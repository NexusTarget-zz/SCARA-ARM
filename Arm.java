...
public class Arm
{
    ...
    public void directKinematic(){
        ...
        double dist = (yj1 - yj2);
        double dist2 = (xj2 - xj1);
        double d = Math.sqrt(Math.pow(dist, 2) + Math.pow(dist2, 2));
        ...
        if (d<2*r){
            ...
            double alpha = Math.toDegrees(Math.atan((dist)/(dist2)));
            //double alpha = dist/dist2; ;
            // tool position
    ...
    public void inverseKinematic(double xt_new,double yt_new){
        ...
        double alpha1 = Math.PI/2 - Math.atan2(yt-ym1, xm1-xt); 
        ...
        double xj12 = xa1 - h1*Math.cos(alpha1);
        double yj12 = ya1 - h1*Math.sin(alpha1);
        ...
        double dx2 = xt - xm2; 
        double dy2 = yt - ym2;
        double d2 = Math.sqrt(dx2*dx2+dy2*dy2);   double xj12 = xa1 - h1*Math.cos(alpha1);
        double yj12 = ya1 - h1*Math.sin(alpha1);
        if (d2>2*r){
    ...
    //motor voltages of motor 1 and motor 2
    public void motorVoltages(){
        motor1 = (int) ((theta1*180/Math.PI - 18.977) / -0.1012);
        motor2 = (int) ((theta2*180/Math.PI - 71.494) /  -0.0966);
    }
    ...
    // linear intepolation
    public int get_pwm1(){
        int pwm = (int) ((theta1 - 18.977) / -0.1012);   //change this to the gradient of pwm1 measurements
        return pwm;
    }
    // ditto for motor 2
    public int get_pwm2(){
        int pwm = (int) ((theta2 - 71.494) /  -0.0966);   //change this to the gradient of pwm2 measurements
